...
toer_cnstr <- Power(dist, H_0) <= alpha
pow_cnstr <- Power(dist, H_1) >= min_power
```
...
The initial designs are chosen heuristically.

```{r}
tbl_designs <- tibble(
    type    = c("one-stage", "group-sequential", "two-stage"),
    initial = list(
        OneStageDesign(121, 3.02),
        GroupSequentialDesign(102, 1.2, 4.35, 102, 
                              c2_pivots=c(4.87, 4.55, 4.01, 3.38, 2.74, 2.21, 1.88)),
        TwoStageDesign(102, 1.2, 4.35, 
                       n2_pivots = c(137, 130, 117, 102, 87, 74, 65), 
                       c2_pivots = c(4.87, 4.55, 4.01, 3.38, 2.74, 2.21, 1.88), 
                       event_rate = 0.7)) )
```
