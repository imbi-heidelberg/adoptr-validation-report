...
Thus, the maximal sample size should not become larger than $105$.
```{r}
...
### Optimization
```{r}
opt_twostage <- minimize(ess,subject_to(toer_cnstr,pow_cnstr,smplsize_cnstr),initial_design = init_twostage)
```

### Testcases

First, we verify the type one error rate and power constraints by simulation:

```{r}
toer <- sim_pr_reject(opt_twostage,0.0,datadist)
power <- sim_pr_reject(opt_twostage,0.5,datadist)

testthat::expect_true(toer<=alpha*(1+tol))
testthat::expect_true(power>=min_power*(1-tol))
```

The $n_2$ function is not necessarily monotonously decreasing, but at least it should be non-decreasing:
```{r}
